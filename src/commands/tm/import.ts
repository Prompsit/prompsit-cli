...
import { JobError, NetworkError } from "../../errors/contracts.ts";
  ...
  tmCommand
    ...
    .action(async (fileArg, opts) => {
      try {
        ...
        const job = await client.tm.importTmx(filePath, opts.profile);
        const completion = await trackJob(client, job.job_id, { description: filePath });
        const result = completion.tmImportResult;
        if (!result) {
          throw new JobError("Server did not provide tm_import_result for a completed TMX import");
        }

        terminal.success(t("tm.import.success", { count: String(result.segment_count) }));
        terminal.line(
          t("tm.import.pair", { source: result.source_lang, target: result.target_lang })
        );
      } catch (error: unknown) {
