...
| **8** | **Security by Design** | code | Security integrated from design phase, not bolted on later | **Practices:** Never commit secrets → env vars/secret managers, Validate at boundaries → Zod schemas, Least Privilege → minimum permissions, Fail Securely → don't leak info in errors, Defense in Depth → multiple security layers |
...
- ✅ Service layer returns DTOs/Zod-validated types, hides internal details
...
**Template Last Updated:** 2026-02-26
